...
async function buyPosition(
    ...
    if (stock.forecast < minimumForecase) {
        //ns.printf("Stock %s below forecast, skipping", stock.symbol);
        return;
    }
...
async function getMinimumShares(
    ...
    while (true) {
        ...
        if (sharesToBuy >= maximumShares) {
            ns.printf("Maximum Shares can be purchased for %s", symbol);
